...
BEGIN
  ...
  -- CHANGED: Count rows in ah_messages directly. 
  -- The 'messages_count' column in conversations seems stale (16 vs 34 real messages).
  -- Counting the table directly is the only way to be 100% accurate.
  SELECT count(*) INTO total_messages FROM public.ah_messages;
  
  -- Sum tokens from conversations (assuming this is the best source we have for now)
